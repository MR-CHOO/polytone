 ...
 *     vec4  SurfaceBiomePalette[MAX_SLOTS * MAX_ATTRIBUTES];  // [slot * attrCount + i]
 * };
 * </pre>
 *
...
public class SurfaceBiomePalette implements AutoCloseable {
    ...
    private static final int UBO_SIZE = paletteSize();
    ...
    private final List<Holder<Biome>> bySlot = new ArrayList<>();
    ...
    public int slotFor(Holder<Biome> biome) {
        ...
        if (bySlot.size() + 1 >= MAX_SLOTS) return OVERFLOW_SLOT;
        bySlot.add(biome);
        int slot = bySlot.size(); // 1-based; 0 stays "not filled yet"
    ...
    public void update(ClientLevel level, Vec3 camPos, List<EnvironmentAttribute<?>> attributes,
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            Std140Builder builder = Std140Builder.onStack(stack, UBO_SIZE)
                    ...
                    .putIVec4(attrCount, bySlot.size(), 0, 0);
            for (int slot = 1; slot <= MAX_SLOTS - 1; slot++) {
                Holder<Biome> biome = slot <= bySlot.size() ? bySlot.get(slot - 1) : null;
                for (int i = 0; i < SurfaceMapSettings.BiomeLayer.MAX_ATTRIBUTES; i++) {
                    if (biome == null || i >= attrCount) {
